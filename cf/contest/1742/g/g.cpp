...
n个数，其实只要确定前log2(mx)个数即可，剩下数无所谓顺序
...
void solve() {
    ...
    int cur = 0;
    for (int i = 0; i < min((int)log2(mx), n); i++) {
        int t = -1;
        for (int j = 0; j < n; j++) {
            if (a[j].second) continue;

            if (t == -1 || (cur | a[j].first) > (cur | a[t].first)) {
                t = j;
            }
        }
