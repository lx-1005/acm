...
bool is_end_of_hotel(std::string show_tag_utf8, std::string hotel_str_utf8) {
    std::wstring_convert<std::codecvt_utf8<wchar_t>> converter;
    std::wstring wide = converter.from_bytes(show_tag_utf8);
    auto lenth = wide.length();
    return lenth >= 2 &&
           converter.to_bytes(wide.substr(lenth - 2, 2)) == hotel_str_utf8;
}



void solve() {
    cout << is_end_of_hotel(string("李旭酒店"), string("酒店")) << endl;
}
