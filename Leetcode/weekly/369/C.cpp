...
public:
    long long minIncrementOperations(vector<int>& nums, int k) {
        ...
        // dp[i][0]: 让前i个数变成美丽数组,且使nums[i]，的最小操作次数
        // dp[i][1]: 让前i个数变成美丽数组,且nums[i-1]<k，使nums[i-2]>=k的最小操作次数，
        // dp[i][2]: 让前i个数变成美丽数组,且nums[i-1]<k, nums[i-2]<k, 使nums[i-3]>=k，的最小操作次数，
        vector<vector<int> > dp(n + 1, vector<int>(3, inf));
        for (int i = 0; i < 3; ++i) dp[i][0] = dp[i][1] = 0;
        for (int i = 3; i < n; ++i) {
            dp[i][2] = dp[i - 2][0];
            dp[i][1] = dp[i - 1][0];
            dp[i][0] = nums[i - 1] >= k ? 0 : k - nums[i];
        }
