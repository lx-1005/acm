...
bool solve() {
    ...
    if (cnt == 2) {
        ...
        if (remain == ans[0] + ans[1]) return true;
    }
    ...
    // cin >> n;
    // vector<pair<LL, bool>> dif;
    // for (int i = 1; i <= n - 1; ++i) {
    //     cin >> b[i];
    //     dif.emplace_back(b[i] - b[i - 1], true);
    // }
    ...
    // for (auto& [x, f] : dif) {
    //     if (vis[x]) vis[x] = false, f = false, cnt--;
    // }
    ...
    //     LL y = 0;
    //     for (int i = 1; i <= n; ++i) {
    //         if (vis[i]) x.push_back(i);
    //         if (i <= n - 1 && dif[i].second) y = i;
    //     }
    //     if (y == x[0] + x[1]) cout << "YES" << endl;
    //     return;
    // } 
    // cout << "NO" << endl;
}
