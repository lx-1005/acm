...
数字n从右往左移动，统计每个位置处为x的情况个数=左侧个数*右侧个数
例如n=12345，x=3，假设当前位于数字3处，因为3>=x，那么该位置可以取3。左侧可以取[0,12]，右侧可以取[0,45]，
暴力枚举[L, R]，统计每个数x中'2'出现的次数，复杂度O(R*lg(R))


*/

int f(int n, int x) { // 返回[1, n]中数字x出现的次数
    int ans = 0;
    int l = n, r = 0, t = 1; 
    while (l) {
        ans += l / 10 * t;
