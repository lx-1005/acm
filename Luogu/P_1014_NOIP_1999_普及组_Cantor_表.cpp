...
模拟+二分
1. 第一轮共1个数，第二轮2个数，... ，第p轮p个数
2. 第1轮到第p轮，共p*(p+1)/2个数
3. 对于第p轮，根据diff可以O(1)确定第n个数的位置
...
void solve() {
    ...
    while (lb + 1 < rb) {
        int mid = (lb + rb) >> 1;
        if (mid * (mid + 1) / 2 >= n) rb = mid;
        else lb = mid;
    }
    int diff = rb * (rb + 1) / 2 - n;
    if (rb % 2) {
        cout << (1 + diff) << "/" << (rb - diff) << endl;
    } else {
        cout << (rb - diff) << "/" << (1 + diff) << endl;
    }
}
